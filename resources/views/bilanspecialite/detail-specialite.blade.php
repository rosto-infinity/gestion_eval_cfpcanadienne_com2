...
@section('content')
    ...
    <div class="card mb-6 no-print bg-card border border-border rounded-lg shadow-sm">
        <div class="p-6">
            <form method="GET" action="{{ route('bilan.specialite.show', $specialite) }}"
            ...
            <div class="overflow-x-auto">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-muted/50 border-b border-border">
                        <tr>
                            <th rowspan="3"
                                class="px-4 py-3 text-center font-bold text-foreground border-r border-border"
                                style="min-width: 40px;">
                                N°
                            </th>
                            <th rowspan="3" class="px-4 py-3 text-left font-bold text-foreground border-r border-border"
                                style="min-width: 220px;">
                                <div>Nom et prénoms</div>
                                <div class="text-xs font-normal text-muted-foreground">Names and first names</div>
                            </th>
                            <th colspan="12"
                                class="px-4 py-3 text-center font-bold text-foreground border-r border-border bg-primary/5">
                                Évaluations semestrielles (30%)
                            </th>

                            <th rowspan="3"
                                class="px-4 py-3 text-center font-bold text-foreground border-r border-border bg-primary/10"
                                style="min-width: 100px;">
                                <div>Bilan des compétences</div>
                                <div class="text-xs font-normal">(70%)</div>
                            </th>
                            <th rowspan="3" class="px-4 py-3 text-center font-bold text-primary bg-primary/10"
                                style="min-width: 100px;">
                                <div>Moy. Gén.</div>
                        ...
                        <tr class="border-b border-border">
                            <th colspan="6"
                                class="px-2 py-2 text-center font-bold text-foreground border-r border-border bg-primary/5">
                                Semestre 1 (M1–M5)
                            </th>
                            <th colspan="6"
                                class="px-2 py-2 text-center font-bold text-foreground border-r border-border bg-primary/5">
                        ...
                        <tr class="border-b border-border">
                            ...
                            @endfor
                            <th
                                class="px-2 py-2 text-center font-bold text-foreground border-r border-border bg-primary/10">
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        @endphp
                        <tr class="hover:bg-muted/30 transition-colors duration-150 {{ $index < 3 ? 'bg-amber-500/5' : '' }}">
                            ...
                            <td class="px-4 py-3 border-r border-border">
                                <div class="flex items-center gap-3">
                                    @if($user->profile)
                                    <img class="h-8 w-8 rounded-full object-cover no-print" src="{{ Storage::url($user->profile) }}" alt="{{ $user->name }}">
                                    @else
                                    <div class="h-8 w-8 rounded-full bg-primary/20 flex items-center justify-center shrink-0">
                                        <span class="text-xs font-bold text-primary">{{ substr($user->name, 0, 1) }}</span>
                                    </div>
                                    @endif
                                    <div>
                                        <div class="font-semibold text-foreground">{{ strtoupper($user->name) }}</div>
                                        <div class="text-xs text-muted-foreground">{{ $user->matricule }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-2 py-3 text-center font-bold text-base border-r border-border bg-primary/10 text-primary">
                                @if (isset($bilan->moy_competences))
                            ...
                            <td class="px-2 py-3 text-center font-bold text-lg bg-primary/10 {{ $isAdmis ? 'text-primary' : 'text-muted-foreground' }}">
                                @if (isset($bilan->moyenne_generale))
                            ...
                            <td class="px-4 py-3 text-center border-r border-border">
                                @if ($isAdmis)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-500/10 text-green-600">✓ Admis</span>
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-destructive/10 text-destructive">✗ Ajourné</span>
                                @endif
                            </td> --}}
                        </tr>
                        @endforeach
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6 pt-10">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-foreground mt-2">
                            {{ number_format($stats['taux_admission'], 1) }}%</p>
                    </div>
                    <div class="bg-primary/10 rounded-full p-3">
                        <svg class="h-8 w-8 text-primary" fill="currentColor" viewBox="0 0 20 20">
                            <path
        ...
        <div class="card bg-primary/5 border border-primary/20 rounded-lg shadow-sm overflow-hidden">
            <div class="p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-primary uppercase tracking-wide">Moye. Générale</p>
                ...
                <div class="flex-1">
                    ...
                    <ul class="space-y-2 text-sm text-muted-foreground">
                        <li class="flex gap-2">
                            ...
                            <span><strong class="text-foreground">Évaluations semestrielles (30%)</strong> : Moyenne des
                                modules M1 à M10</span>
                        </li>
                        <li class="flex gap-2">
                            <span class="text-primary font-bold">•</span>
                            <span><strong class="text-foreground">Moy. S1</strong> = Moyenne des modules M1 à M5 (Semestre
                                1)</span>
                        </li>
                        <li class="flex gap-2">
                            <span class="text-primary font-bold">•</span>
                            <span><strong class="text-foreground">Moy. S2</strong> = Moyenne des modules M6 à M10 (Semestre
                                2)</span>
                        </li>
                        <li class="flex gap-2">
                            <span class="text-primary font-bold">•</span>
                            <span><strong class="text-foreground">Bilan des compétences (70%)</strong> = Évaluation
                                pratique des compétences</span>
                        </li>
                        <li class="flex gap-2">
                            <span class="text-primary font-bold">•</span>
                            <span><strong class="text-foreground">Moy. Gén. (100%)</strong> = [(Moy. S1 + Moy. S2) / 2 ×
                                30%] + [Bilan compétences × 70%]</span>
                        </li>
                    </ul>
                </div>
            </div>
