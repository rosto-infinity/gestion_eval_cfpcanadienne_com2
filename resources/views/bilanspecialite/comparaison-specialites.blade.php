...
@section('content')
    <div class="min-h-screen" style="background-color: var(--background)">
        ...
        <!-- Filtres -->
        <div class="mb-6 rounded-xl border-2 overflow-hidden transition-all duration-200 hover:shadow-md no-print"
                ...
                <form method="GET" action="{{ route('bilan.specialite.comparaison') }}" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="space-y-2 max-h-48 overflow-y-auto border-2 rounded-lg p-4"
                                ...
                                @foreach ($specialites as $specialite)
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        ...
                                        <span class="text-sm font-medium transition-all duration-200 group-hover:opacity-75"
        ...
        @else
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <!-- Graphique radar -->
                <div class="rounded-xl border-2 overflow-hidden transition-all duration-200 hover:shadow-md"
                    style="background-color: var(--card); border-color: var(--border)">
            ...
            <div class="rounded-xl border-2 overflow-hidden transition-all duration-200 hover:shadow-md mb-6"
                style="background-color: var(--card); border-color: var(--border)">
                <div class="p-6 border-b-2" style="border-color: var(--border)">
                    <h3 class="text-lg font-bold flex items-center gap-2" style="color: var(--foreground)">
                        <span style="color: var(--primary)">📈</span>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr style="border-bottom: 2px solid var(--border)">
                                ...
                                <th class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider"
                                    style="color: var(--muted-foreground); background-color: var(--secondary)">
                                    Moy. S1
                                </th>
                                <th class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider"
                                    style="color: var(--muted-foreground); background-color: var(--secondary)">
                                ...
                                <th class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider"
                                    style="color: var(--primary); background-color: var(--secondary); font-weight: 900">
                                    Moy. Gén.
                                </th>
                                <th class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider"
                                    style="color: var(--muted-foreground); background-color: var(--secondary)">
                        ...
                        <tbody style="border-color: var(--border)">
                            @foreach ($bilanParSpecialite as $index => $bilan)
                                <tr class="border-b transition-all duration-200 hover:opacity-75"
                                    style="border-color: var(--border)">
                                    <!-- Spécialité -->
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            <div class="h-10 w-10 rounded-lg flex items-center justify-center text-xs font-bold text-white"
...
<script>
    ...
    @if ($bilanParSpecialite->isNotEmpty())
        ...
        // Fallback si les couleurs CSS ne sont pas chargées
        if (!primaryColor) primaryColor = '#ef4444'; // Rouge par défaut
        if (!secondaryColor) secondaryColor = '#e2e8f0'; // Gris par défaut
        if (!mutedColor) mutedColor = '#64748b'; // Gris par défaut
        ...
        const chartOptions = {
            responsive: true,
            maintainAspectRatio: false, // Utilisé false pour les graphiques radar pour mieux remplir
            plugins: {
                legend: {
                    labels: {
                        color: mutedColor,
            ...
            new Chart(ctxLine, {
                ...
                data: {
                    ...
                    datasets: [{
                            ...
                            pointBorderWidth: 2,
                            pointHoverRadius: 7,
                            yAxisID: 'y' // Correctement placé ici pour lier au dessous
                        },
                        {
                            ...
                            pointBorderWidth: 2,
                            pointHoverRadius: 7,
                            yAxisID: 'y1' // Correctement placé ici pour lier à l'axe de droite
                        }
                    ]
                },
                options: {
                    ...
                    scales: {
                        // Axe de GAUCHE (Moyenne Générale)
                        y: {
                            type: 'linear',
                            display: true,
                            position: 'left',
                            id: 'y', // CORRECTION : On donne l'ID ici
                            title: {
                                display: true,
                        ...
                        // Axe de DROITE (Taux Admission)
                        y1: {
                            type: 'linear',
                            display: true,
                            position: 'right',
                            id: 'y1', // CORRECTION : On donne l'ID ici
                            title: {
                                display: true,
                            ...
                            }, // Ne pas afficher la grille sur le graphique
                            ticks: {
                                color: mutedColor,
                                font: {
                                    size: 11
