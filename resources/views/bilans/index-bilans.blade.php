<x-app-layout title="Bilans de Compétences">
    <div class=" mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="mb-6 flex justify-between items-center">
            ...
            <a href="{{ route('bilans.create') }}"
                class="px-4 py-2 text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 rounded-md transition-colors inline-flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
                Nouveau bilan
            </a>
            <a href="{{ route('bilans.tableau-recapitulatif') }}"
                class="px-4 py-2 text-sm font-medium text-primary-foreground bg-primary/50 hover:bg-primary/90 rounded-md transition-colors inline-flex items-center gap-2">
        ...
        <div class="bg-card border border-border rounded-lg p-5 mb-6">
            <form method="GET" action="{{ route('bilans.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-end">
                    <button type="submit"
                        class="w-full px-4 py-2 text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 rounded-md transition-colors inline-flex items-center justify-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @foreach ($bilans as $bilan)
                                <tr class="hover:bg-muted/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <span
                                            class="text-sm font-medium text-foreground">{{ $bilan->user->matricule ?? '-' }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="text-sm font-medium text-foreground">{{ $bilan->user->name }}
                                                </div>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-center">
                                        <div
                                            class="text-sm font-bold {{ $bilan->moyenne_generale >= 10 ? 'text-green-600' : 'text-destructive' }}">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <!-- Voir -->
                                            <a href="{{ route('bilans.show', $bilan) }}"
                                            ...
                                            <!-- Modifier -->
                                            <a href="{{ route('bilans.edit', $bilan) }}"
                                                class="p-1.5 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded transition-colors"
                                                title="Modifier">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                            ...
                                            @if (auth()->user()->role->isAtLeast(\App\Enums\Role::ADMIN))
                                                <form action="{{ route('bilans.destroy', $bilan) }}" method="POST"
                                                    class="inline" onsubmit="return confirm('Supprimer ce bilan ?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
                ...
                <div class="bg-card border border-border rounded-lg p-5">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Total de bilans
                            </p>
                        ...
                        <div class="p-2.5 bg-primary/10 rounded-lg">
                            <svg class="w-5 h-5 text-primary" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <path fill-rule="evenodd"
                                    d="M3 10a1 1 0 011-1h12a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zm5-3a1 1 0 000 2h2a1 1 0 100-2H8z"
                                    clip-rule="evenodd" />
                            </svg>
                        </div>
                    </div>
                ...
                <div class="bg-card border border-border rounded-lg p-5">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Moyenne générale
                            </p>
                            <p class="mt-2 text-2xl font-bold text-primary">
                                {{ number_format($bilans->avg('moyenne_generale'), 2) }}/20</p>
                        </div>
                        <div class="p-2.5 bg-primary/10 rounded-lg">
                            <svg class="w-5 h-5 text-primary" fill="currentColor" viewBox="0 0 20 20">
                                <path
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-2xl font-bold text-green-600">
                                {{ $bilans->where('moyenne_generale', '>=', 10)->count() }}</p>
                        </div>
                ...
                <div class="bg-card border border-border rounded-lg p-5">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Ajourné (<10)<
                                    /p>
