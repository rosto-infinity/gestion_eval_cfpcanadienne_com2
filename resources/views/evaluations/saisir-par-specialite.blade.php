                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <div id="moduleLoading" class="hidden">
                            <div
                                class="w-full px-4 py-2.5 rounded-lg border border-border bg-muted flex items-center gap-2">
                        ...
                        <select name="module_id" id="module_id"
                            ...
                            <option value="">
                                {{ $specialite ? '-- Sélectionner --' : '-- Sélectionner d\'abord une spécialité --' }}
                            </option>
                            @foreach ($modules as $mod)
                                <option value="{{ $mod->id }}" data-code="{{ $mod->code }}"
                    ...
                    <div class="flex items-end gap-2">
                        <a href="{{ route('saisir-par-specialite') }}"
                            class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-secondary hover:bg-secondary/80 text-secondary-foreground rounded-lg font-medium transition-all"
                        ...
                        <button type="submit"
                            class="flex-1 inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-medium transition-all">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground mt-1">
                            Spécialité: {{ $specialite->intitule }} | Semestre {{ $semestre }} | Coefficient:
                            {{ $module->coefficient }}
                        </p>
                    </div>
                    ...
                    <div class="bg-muted px-6 py-4 border-b border-border">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-muted-foreground mt-1">{{ $students->count() }} étudiant(s)
                                    trouvé(s)</p>
                            </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-muted/50">
                                <tr>
                                    ...
                                    <th
                                        class="px-6 py-3 text-left text-xs font-bold text-muted-foreground uppercase tracking-wider w-48">
                                        Note (/20)
                                    </th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-bold text-muted-foreground uppercase tracking-wider">
                            ...
                            <tbody class="divide-y divide-border">
                                @foreach ($students as $index => $student)
                                    <tr class="hover:bg-muted/30 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="relative">
                                                <input type="number" name="notes[{{ $student->user->id }}]"
                                                    id="note_{{ $student->user->id }}" min="0" max="20"
                                                    step="0.01" value="{{ $student->note }}"
                                                    ...
                                                    placeholder="0.00"
                                                    onchange="updateRowAppreciation({{ $student->user->id }})" required>
                                                <span
                                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">/20</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span id="appreciation_{{ $student->user->id }}"
                                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold">
                                                @if ($student->has_evaluation)
                                                    @if ($student->note >= 10)
                                                        <span
                                                            class="bg-green-100 text-green-700 px-2.5 py-1 rounded-full">✓
                                                            Admis</span>
                                                    @else
                                                        <span class="bg-red-100 text-red-700 px-2.5 py-1 rounded-full">✗
                                                            Échec</span>
                                                    @endif
                                                @else
                                                    <span class="bg-gray-100 text-gray-700 px-2.5 py-1 rounded-full">○
                                                        Nouveau</span>
                                                @endif
                                            </span>
                                        </td>
                    ...
                    <div class="bg-muted/30 px-6 py-4 border-t border-border">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div class="bg-background rounded-lg p-4">
                                ...
                                <p class="text-2xl font-bold text-foreground" id="stat_total">{{ $students->count() }}
                                </p>
                            </div>
                ...
                .then(data => {
                    ...
                    if (data.success && data.modules.length > 0) {
                        data.modules.forEach(module => {
                            ...
                            option.textContent =
                                `${module.code} - ${module.intitule} (Coef. ${module.coefficient})`;
