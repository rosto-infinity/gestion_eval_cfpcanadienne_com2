...
<div x-data>
    @if($type === 'pill')
        <label :class="$store.theme.darkMode ? 'bg-primary' : 'bg-gray-600'"
            class="relative m-0 block h-7.5 w-14 rounded-full cursor-pointer select-none">
            <input type="checkbox" :checked="$store.theme.darkMode" @click.prevent="$store.theme.toggle($event)"
                class="absolute top-0 z-50 m-0 h-7.5 w-14 cursor-pointer opacity-0" />
            <span :class="$store.theme.darkMode && 'right-1! translate-x-full!'"
    ...
    @else
        <button @click="$store.theme.toggle($event)" type="button"
            ...
            <template x-if="!$store.theme.darkMode">
                <i class="bx bx-sun text-yellow-500 text-xl"></i>
            </template>
            <template x-if="$store.theme.darkMode">
